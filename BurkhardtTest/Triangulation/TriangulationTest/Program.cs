...
internal static class Program
{
        ...
        //
    {
        ...
        for (test = 0; test < TEST_NUM; test++)
        {
            ...
            for (node = 0; node < NODE_NUM; node++)
            {
                switch (node)
                {
                    case >= 1 when node_rep[node - 1] == node_rep[node]:
                        continue;
                }
    ...
    private static double[] quad_fun(int n, double[] xy_vec, double[] f_vec )
