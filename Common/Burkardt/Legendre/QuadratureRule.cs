...
namespace Burkardt.Legendre
{
    ...
    {
        public static void lg_abscissa(int dim_num, int point_num, int[] grid_index,
                int[] grid_base, ref double[] grid_point, int gridIndex = 0, int gridBaseIndex = 0, int gridPtIndex = 0)
            ...
            //
        {
            ...
            {
                if (grid_base[((dim) + gridBaseIndex) % grid_base.Length] < 1)
                {
                    Console.WriteLine("");
                    Console.WriteLine("LG_ABSCISSA - Fatal error!");
            ...
            for (dim = 0; dim < dim_num; dim++)
            {
                if (127 < grid_base[((dim) + gridBaseIndex) % grid_base.Length])
                {
                    Console.WriteLine("");
                    Console.WriteLine("LG_ABSCISSA - Fatal error!");
            ...
            for (point = 0; point < point_num; point++)
            {
                for (dim = 0; dim < dim_num; dim++)
                {
                    ...
                    if (pointer < 1 || 247 < pointer)
                    {
                        ...
                        Console.WriteLine("  GRID_BASE  = " + grid_base[((dim) + gridBaseIndex) % grid_base.Length] + "");
                        Console.WriteLine("  LEVEL      = " + level + "");
                        Console.WriteLine("  GRID_INDEX = " + grid_index[((dim + point * dim_num) + gridIndex) % grid_index.Length] + "");
                        return;
                    }

                    grid_point[((dim + point * dim_num) + gridPtIndex) % grid_point.Length] = x[pointer - 1];
                }
            }

        }

        public static double[] lg_weights(int order)
            ...
            //
        {
            ...
            {
                Console.WriteLine("");
                Console.WriteLine("LG_WEIGHTS - Fatal error!");
