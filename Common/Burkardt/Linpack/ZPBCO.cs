            ...
            //
        {
            ...
            for (k = 1; k <= n; k++)
            {
                ...
                if (k + 1 <= j2)
                {
                    ...
                    {
                        i = i - 1;
                        sm = sm + typeMethods.zabs1(z[j - 1] + wkm * Complex.Conjugate(abd[i - 1 + (j - 1) * lda]));
                        z[j - 1] = z[j - 1] + wk * Complex.Conjugate(abd[i - 1 + (j - 1) * lda]);
                    ...
                    if (s < sm)
                    {
                        ...
                        {
                            i = i - 1;
                            z[j - 1] = z[j - 1] + t * Complex.Conjugate(abd[i - 1 + (j - 1) * lda]);
                        }
                    }
                }
            ...
            for (k = 1; k <= n; k++)
            {
                ...
                la = m + 1 - lm;
                lb = k - lm;
                z[k - 1] = z[k - 1] - BLAS1Z.zdotc(lm, abd, 1, z, 1, xIndex: +la - 1 + (k - 1) * lda, yIndex: +lb - 1);
