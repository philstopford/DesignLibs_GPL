...
namespace Burkardt.Linpack
{
    ...
    {
        public static double zsico(ref Complex[] a, int lda, int n, int[] ipvt)
            ...
            //
        {
            ...
            while (k <= n)
            {
                ...
                if (k != 1)
                {
                    z[k - 1] = z[k - 1] + BLAS1Z.zdotu(k - 1, a, 1, z, 1, xIndex: +0 + (k - 1) * lda);

                    if (ks == 2)
                    {
                        z[k] = z[k] + BLAS1Z.zdotu(k - 1, a, 1, z, 1, xIndex: +0 + k * lda);
                    }
