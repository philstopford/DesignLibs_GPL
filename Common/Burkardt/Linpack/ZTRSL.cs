...
namespace Burkardt.Linpack
{
    public static class ZTRSL
    {
            ...
            //
        {
            ...
            {
                b[n - 1] = b[n - 1] / Complex.Conjugate(t[n - 1 + (n - 1) * ldt]);

                for (jj = 2; jj <= n; jj++)
                {
                    j = n - jj + 1;
                    b[j - 1] = b[j - 1] - BLAS1Z.zdotc(jj - 1, t, 1, b, 1, xIndex: +j + (j - 1) * ldt, yIndex: +j);
                    b[j - 1] = b[j - 1] / Complex.Conjugate(t[j - 1 + (j - 1) * ldt]);
                }
            }
            //
            ...
            {
                b[0] = b[0] / Complex.Conjugate(t[0 + 0 * ldt]);

                for (j = 2; j <= n; j++)
                {
                    b[j - 1] = b[j - 1] - BLAS1Z.zdotc(j - 1, t, 1, b, 1, xIndex: +0 + (j - 1) * ldt);
                    b[j - 1] = b[j - 1] / Complex.Conjugate(t[j - 1 + (j - 1) * ldt]);
                }
            }
