    ...
    public static class ODE
    {
            ...
            //
        {
            ...
            for (j = 1; j <= ki; j++)
            {
                ...
                for (k = 0; k < neqn; k++)
                {
                    yout[(k + youtIndex) % yout.Length] = yout[(k + youtIndex) % yout.Length] +
                                                          g[i - 1] * phi[
                                                              ((k + (i - 1) * neqn) + phiIndex) % phi.Length];
                    ypout[(k + ypoutIndex) % ypout.Length] = ypout[(k + ypoutIndex) % ypout.Length] +
                                                             rho[i - 1] *
            ...
            //
        {
            ...
            for (i = 0; i < neqn; i++)
            {
                round = round + Math.Pow(y[(i + yIndex) % y.Length] / wt[(i + wtIndex) % wt.Length], 2);
            }
            ...
            if (start)
            {
                ...
                for (l = 1; l <= neqn; l++)
                {
                    total = total + Math.Pow(yp[((l - 1) + ypIndex) % yp.Length] / wt[((l - 1) + wtIndex) % wt.Length],
                        2);
                }
            ...
            for (;;)
            {
                ...
                if (ns <= k)
                {
                    ...
                    alpha[((ns - 1) + alphaIndex) % alpha.Length] = 1.0 / (double) (ns);
                    temp1 = h * (double) (ns);
                    ...
                    for (i = nsp1; i <= k; i++)
                    {
                        ...
                        sig[((i) + sigIndex) % sig.Length] = (double) (i) *
                    ...
                    if (ns <= 1)
                    {
                        for (iq = 1; iq <= k; iq++)
                        {
                            v[((iq - 1) + vIndex) % v.Length] = 1.0 / (double) (iq * (iq + 1));
                    ...
                    else
                    {
                        if (kold < k)
                        {
                            v[((k - 1) + vIndex) % v.Length] = 1.0 / (double) (k * kp1);

                            for (j = 1; j <= ns - 2; j++)
                            {
                                i = k - j;
                                v[((i - 1) + vIndex) % v.Length] = v[((i - 1) + vIndex) % v.Length] -
                                                                   alpha[((j) + alphaIndex) % alpha.Length] *
                                                                   v[((i) + vIndex) % v.Length];
                            }
                        }
                        ...
                        {
                            v[((iq - 1) + vIndex) % v.Length] = v[((iq - 1) + vIndex) % v.Length] -
                                                                alpha[((ns - 1) + alphaIndex) % alpha.Length] *
                    ...
                    for (i = ns + 2; i <= kp1; i++)
                    {
                        ...
                        {
                            w[((iq - 1) + wIndex) % w.Length] = w[((iq - 1) + wIndex) % w.Length] -
                                                                alpha[((i - 2) + alphaIndex) % alpha.Length] *
                ...
                for (j = 1; j <= k; j++)
                {
                    ...
                    for (l = 1; l <= neqn; l++)
                    {
                        p[((l - 1) + pIndex) % p.Length] = p[((l - 1) + pIndex) % p.Length] +
                                                           g[((i - 1) + gIndex) % g.Length] *
                                                           phi[((l - 1 + (i - 1) * neqn) + phiIndex) % phi.Length];
                        phi[((l - 1 + (i - 1) * neqn) + phiIndex) % phi.Length] =
                            phi[((l - 1 + (i - 1) * neqn) + phiIndex) % phi.Length] +
                ...
                if (0 < km2)
                {
                    erkm2 = absh * sig[((km1 - 1) + sigIndex) % sig.Length] * gstr[km2 - 1] * Math.Sqrt(erkm2);
                }

                if (0 <= km2)
                {
                    erkm1 = absh * sig[((k - 1) + sigIndex) % sig.Length] * gstr[km1 - 1] * Math.Sqrt(erkm1);
                }
