...
//
disfo = Burkardt.PointsNS.Geometry.points_dist_3d ( pf, po+i*3 );
...
//
alpha = Angle.angle_rad_3d ( po+i*3, pf, pn );
...
//
t = - ( a * p[(0 + pIndex) % p.Length] + b * p[(1 + pIndex) % p.Length] + c * p[(2 + pIndex) % p.Length] + d ) / ( a * a + b * b + c * c );
...
double c, double d, ref double dist, ref double[] pnp, ref double[] pnl )
...
dist = Math.Abs ( t1 );
typeMethods.r8vec_copy ( DIM_NUM, p1, ref pnl );
...
{
typeMethods.r8vec_copy ( DIM_NUM, p2, ref pnl );
dist = t2;
pnp[0] = p2[0] - an * t2;
pnp[1] = p2[1] - bn * t2;
pnp[2] = p2[2] - cn * t2;
...
{
plane_imp_triangle_int_add_3d ( t+0*3, t+2*3, dist1, dist3, ref n, p );
}
else if ( dist3 == 0.0 )
{
plane_imp_triangle_int_add_3d ( t+0*3, t+1*3, dist1, dist2, ref n, p );
}
return;
}
//
...
{
plane_imp_triangle_int_add_3d ( t+1*3, t+0*3, dist2, dist1, ref n, p );
plane_imp_triangle_int_add_3d ( t+1*3, t+2*3, dist2, dist3, ref n, p );
}
else if ( dist3 * dist1 < 0.0 && dist3 * dist2 < 0.0 )
{
plane_imp_triangle_int_add_3d ( t+2*3, t+0*3, dist3, dist1, ref n, p );
plane_imp_triangle_int_add_3d ( t+2*3, t+1*3, dist3, dist2, ref n, p );
}
...
//
v = typeMethods.r8vec_normal_01_new ( DIM_NUM, seed );
