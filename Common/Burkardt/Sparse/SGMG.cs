            ...
            //
        {
            ...
            for (dim = 0; dim < dim_num; dim++)
            {
                for (level = 0; level <= level_max; level++)
                {
                    ...
                    {
                        points = Burkardt.Legendre.QuadratureRule.legendre_compute_points_np(
                            order, np[dim], p.Skip(+p_index).ToArray(), points);
                    }
                    else if (rule[dim] == 5)
                    {
                    ...
                    {
                        points = Burkardt.Laguerre.QuadratureRule.laguerre_compute_points_np(
                            order, np[dim], p.Skip(+p_index).ToArray(), points);
                    }
                    else if (rule[dim] == 8)
                    {
                        points = Burkardt.Laguerre.QuadratureRule.gen_laguerre_compute_points_np(
                            order, np[dim], p.Skip(+p_index).ToArray(), points);
                    }
                    else if (rule[dim] == 9)
                    {
