    ...
    public static partial class typeMethods
    {
            ...
            //
        {
            ...
            for (jcol = 1; jcol <= n; jcol++)
            {
                ...
                x[((jcol - 1) + xIndex ) % x.Length] = x[((jcol - 1) + xIndex ) % x.Length] / t;
                //
                ...
                for (i = jcol + 1; i <= n; i++)
                {
                    if (a2[i - 1 + (jcol - 1) * n] != 0.0)
                    {
                        ...
                        x[((i - 1) + xIndex ) % x.Length] = x[((i - 1) + xIndex ) % x.Length] + t * x[((jcol - 1) + xIndex ) % x.Length];
                    }
                }
            }
            ...
            for (jcol = n; 2 <= jcol; jcol--)
            {
                for (i = 1; i < jcol; i++)
                {
                    x[((i - 1) + xIndex ) % x.Length] = x[((i - 1) + xIndex ) % x.Length] - a2[i - 1 + (jcol - 1) * n] * x[((jcol - 1) + xIndex ) % x.Length];
                }
            }
