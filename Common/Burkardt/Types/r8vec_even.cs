...
public static partial class typeMethods
{
    ...
    public static double r8vec_even_select(int n, double xlo, double xhi, int ival)
        ...
        //
    {
        double xval = n switch
        {
            1 => 0.5 * (xlo + xhi),
            _ => ((n - ival) * xlo + (ival - 1) * xhi) / (n - 1)
        };
        ...
        //
    {
        ...
        for (i = 1; i <= nold - 1; i++)
        {
            ...
            int nadd = nfill[i - 1] + 2;

            int j;
            for (j = 1; j <= nadd; j++)
            {
                xval[nval + j - 2] = ((nadd - j) * xold[i - 1]
                                      + (j - 1) * xold[i])
                                     / (nadd - 1);
            }
        ...
        //
    {
        ...
        for (i = 1; i <= nold - 1; i++)
        {
            xlen += Math.Abs(xold[i] - xold[i - 1]);
        }

        int ntemp = nval - nold;

        double density = ntemp / xlen;
        ...
        for (i = 1; i <= nold - 1; i++)
        {
            double xleni = Math.Abs(xold[i] - xold[i - 1]);
            int npts = (int) (density * xleni);
            ntot += npts;
            //
            ...
            //
            xlentot += xleni;
            int nmaybe = (int)r8_nint(xlentot * density);
            ...
            for (j = 1; j <= npts + 2; j++)
            {
                xval[ival + j - 2] = ((npts + 2 - j) * xold[i - 1]
                                      + (j - 1) * xold[i])
                                     / (npts + 2 - 1);
            }
