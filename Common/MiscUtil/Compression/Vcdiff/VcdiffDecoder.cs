...
public sealed class VcdiffDecoder
{
    ...
    /// </summary>
    private AddressCache cache = new(4, 3);
    #endregion
    ...
    public static void Decode(Stream original, Stream delta, Stream output)
    {
        ...
        VcdiffDecoder instance = new(original, delta, output);
        instance.Decode();
    }
    #endregion
    ...
    private void ReadCodeTable()
    {
        ...
        MemoryStream tableOriginal = new(defaultTableData, false);
        MemoryStream tableDelta = new(compressedTableData, false);
        byte[] decompressedTableData = new byte[1536];
        MemoryStream tableOutput = new(decompressedTableData, true);
    ...
    private bool DecodeWindow()
    {
        ...
        MemoryStream targetDataStream = new(targetData, true);
        ...
        MemoryStream instructionStream = new(instructions, false);
        ...
        while (true)
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                size = size switch
                {
                    0 when instruction.Type != InstructionType.NoOp => IOHelper.ReadBigEndian7BitEncodedInt(
                        instructionStream),
                    _ => size
                };
