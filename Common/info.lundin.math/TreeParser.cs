...
public class TreeParser
{
    ...
    public TreeParser(IDictionary<string, Operator> operators, IDictionary<string, double> constants)
    {
        ...
        foreach (KeyValuePair<string, Operator> pair in operators)
        {
    ...
    public CultureInfo Culture
    {
        ...
        set
        {
            ...
            switch (value.NumberFormat.NumberDecimalSeparator)
            {
            ...
            try
            {
                ...
                if ((op = getOp(exp, i)) != null)
                {
                    ...
                    switch (bRequireParentheses)
                    {
                        ...
                        case true when !isTwoArgOp(op) && op != "!" && exp[i] != '(':
    ...
    {
        switch (bImplicitMultiplication)
        {
        ...
        while (i < l)
        {
            try
            {
                ...
                else
                {
                    switch (exp[i])
                    {
                        case '(' when i - 1 >= 0 && isConstant(exp[i - 1]):
                            // case: "const jp ( expr )" , 2(3+x)
                            str.Insert(i + p, "*");
                            p++;
                            break;
                        default:
                        {
                            if (isAlpha(exp[i]) && i - 1 >= 0 && exp[i - 1] == ')')
                            {
                                // case: ( expr ) jp variable or one-arg-op , (2-x)x , (2-x)sin(x)
                                str.Insert(i + p, "*");
                                p++;
        ...
        ParseE(string exp)
    {
        ...
        while (i < len)
        {
            try
            {
                switch (exp[i])
                {
                    case 'e' when i - 1 >= 0 && char.IsDigit(exp[i - 1]):
                    {
        ...
        ParseInfix(string exp)
    {
        ...
        switch (exp[0])
        {
            case '(' when (ma = match(exp, 0)) == len - 1:
        ...
        while (i < len)
        {
            ...
            else
            {
                if (isTwoArgOp(fop))
                {
                    ...
                    tree = tree switch
                    {
                        null when fop.Equals("+") || fop.Equals("-") => new Node(0D),
                        null => throw new Exception("Wrong number of arguments to operator " + fop),
                        _ => tree
                    };
        ...
        skipSpaces(string str)
    {
        ...
        while (i < len)
        {
            switch (char.IsWhiteSpace(str[i]))
            {
                case false:
                    nstr.Append(str[i]);
                    break;
        ...
        getOp(string exp, int index)
    {
        ...
        for (i = 0; i < maxoplength; i++)
        {
            switch (index)
            {
                case >= 0 when index + maxoplength - i <= len:
                {
        ...
        arg(string _operator, string exp, int index)
    {
        ...
        prec = _operator switch
        {
            null => -1,
            _ => operators[_operator].Precedence
        };
